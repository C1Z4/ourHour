...
import static com.ourhour.domain.org.exception.OrgException.*;
    ...
    public void assertNotLastRootAdminAcrossAll(List<MemberEntity> memberEntityList) {
        ...
        for (MemberEntity member : memberEntityList) {
            List<OrgParticipantMemberEntity> orgParticipantMemberEntityList = orgParticipantMemberRepository
                    .findAllByMemberEntity_MemberIdAndStatus(member.getMemberId(), Status.ACTIVE);

            // 루트 관리자가 하나인 회사가 하나라도 존재하는 경우 계정 탈퇴 불가
            for (OrgParticipantMemberEntity opm : orgParticipantMemberEntityList) {
                if (opm.getRole() != Role.ROOT_ADMIN)
                    continue;

                // 루트 관리자인 경우 해당 회사의 루트 관리자 수 조회
                Long orgId = opm.getOrgEntity().getOrgId();
                int rootAdminCount = orgParticipantMemberRepository.countByOrgEntity_OrgIdAndRoleAndStatus(orgId,
        ...
        if (!blockedOrgList.isEmpty()) {
            throw AuthException.deleteUserException(String.join(", ", blockedOrgList));
        }
