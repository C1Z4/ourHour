...
import { useOrgMemberListAllQuery } from '../org/useOrgQueries';

// 채팅방 생성 시 나를 제외한 멤버들을 초대 목록에 보여줄 때
export const useOrgMembersWithoutMe = (orgId: number) => {
  const { data: allMembers, isLoading, isError } = useOrgMemberListAllQuery(orgId);
...
export const useOrgMembersWithoutChatParticipants = (orgId: number, roomId: number) => {
  const { data: allMembers, isLoading: isLoadingAllMembers } = useOrgMemberListAllQuery(orgId);
